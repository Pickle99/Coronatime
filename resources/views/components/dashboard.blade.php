...
<header class="border-b-2 h-20 flex items-center justify-between">
       ...
       <x-dropdown class="mr-5 lg:mr-0"></x-dropdown>
          ...
          <h1 class="lg:ml-10 hidden lg:block font-extrabold text-black">{{auth()->user()->username}}</h1>
           <span class="mr-5 block lg:hidden group">
              <img class="" src="{{asset('storage/images/drop.png')}}" alt="dropdown"/>
    ...
    <div class="mx-4 mt-10 lg:mx-52 lg:mt-10">
        ...
        <div class="flex mt-10 h-10 border-b-2">
                <a href="/dashboard?page=worldwide&{{http_build_query(request()->except('page'))}}" class="{{$page === 'worldwide' || is_null($page) ? 'font-bold border-b-4 border-black text-lg lg:text-sm lg:text-xl mr-20' : 'text-xl mr-20'}}" ><span class="block">{{__('translate.worldwide')}}</span></a>
            <a href="/dashboard?page=country&{{http_build_query(request()->except('page'))}}"  class="{{$page === 'country' ? 'w-fit font-bold border-b-4 border-black text-lg lg:text-xl mr-20' : 'text-lg lg:text-xl mr-20'}}" ><span class="block w-40">{{__('translate.by_country')}}</span></a>
            </div>
    </div>
    <div>
